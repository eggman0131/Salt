...
export const InventoryModule: React.FC<InventoryModuleProps> = ({ inventory, onRefresh }) => {
  ...
  void isAdding; // unused state retained for future use
  ...
  const handleEditSave = async () => {
    ...
    setIsSaving(true);
    try {
      await updateEquipment(equipmentToEdit.id, { status, accessories });
      await onRefresh();
  ...
  const categoryVariant = (category: EquipmentCandidate['category']) => {
    switch (category) {
      case 'Complex Appliance': return 'default' as const;
      case 'Technical Cookware': return 'secondary' as const;
        ...
        <div className="flex-1 min-h-0">
          {inventory.length === 0 ? (
            <div className="py-12 text-center border border-dashed rounded-lg">
              <p className="text-sm text-muted-foreground">No equipment yet. Add equipment above.</p>
            </div>
          ) : (
            <>
                      ...
                      >
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="text-xs text-muted-foreground space-y-1">
                            ...
                            <p><span className="font-medium">Model:</span> {item.modelName}</p>
                            <p><span className="font-medium">Type:</span> {item.type}</p>
                            <p><span className="font-medium">Class:</span> {item.class}</p>
                            {item.accessories && item.accessories.length > 0 && (
                              <p className="pt-1">
                                {item.accessories.filter((a) => a.owned).length} / {item.accessories.length} accessories
                              </p>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center gap-1 pt-2 border-t">
                          <Button onClick={() => handleEditClick(item)} variant="ghost" size="icon" className="text-muted-foreground hover:bg-primary/10 hover:text-primary">
                            <Pencil className="h-4 w-4" />
                          </Button>
                          <Button onClick={() => setEquipmentToDelete(item)} variant="ghost" size="icon" className="text-muted-foreground hover:bg-destructive/10 hover:text-destructive">
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh]">
          <DialogHeader>
            ...
            <DialogDescription>Search our database or add a new item to your kitchen.</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4 pr-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={handleSearch} disabled={!searchQuery.trim() || isSearching || isGenerating} variant="outline">
              ...
              {isSearching ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-3">Searching equipment database...</p>
                </div>
              ) : isGenerating ? (
                <div className="py-12 text-center">
                  <Loader2 className="h-8 w-8 animate-spin mx-auto text-primary" />
                  <p className="text-sm text-muted-foreground mt-3">Generating equipment details...</p>
                </div>
              ) : hasSearched && candidates.length === 0 ? (
                <div className="py-12 text-center border border-dashed rounded-lg">
                  <p className="text-sm text-muted-foreground">No equipment found matching "{searchQuery}"</p>
                  <p className="text-xs text-muted-foreground mt-1">Try a different search term or brand name</p>
                </div>
              ) : candidates.length > 0 ? (
                <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-start gap-2 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm">{candidate.brand} {candidate.modelName}</p>
                          </div>
                          <Badge variant={categoryVariant(candidate.category)} className="text-xs shrink-0">
                            {candidate.category}
                          </Badge>
                        </div>
      ...
      <Dialog open={!!equipmentToEdit} onOpenChange={() => setEquipmentToEdit(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh]">
          <DialogHeader>
            ...
            <DialogDescription>{equipmentToEdit?.brand} • {equipmentToEdit?.modelName}</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4 pr-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  {(['Available', 'In Use', 'Maintenance'] as const).map((s) => (
                    <Button key={s} onClick={() => setStatus(s)} variant={status === s ? 'default' : 'outline'} size="sm">
                      {s}
                    </Button>
                  ))}
                </div>
              ...
              {equipmentToEdit && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div><span className="text-muted-foreground">Brand:</span><p className="font-medium">{equipmentToEdit.brand}</p></div>
                    <div><span className="text-muted-foreground">Model:</span><p className="font-medium">{equipmentToEdit.modelName}</p></div>
                    <div><span className="text-muted-foreground">Type:</span><p className="font-medium">{equipmentToEdit.type}</p></div>
                    <div><span className="text-muted-foreground">Class:</span><p className="font-medium">{equipmentToEdit.class}</p></div>
                  </div>
                  {equipmentToEdit.description && (
                    <p className="text-sm text-muted-foreground mt-2">{equipmentToEdit.description}</p>
                  )}
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 rounded-lg border border-dashed p-3">
                  <Label htmlFor="new-accessory" className="text-xs">Add New Accessory</Label>
                  <div className="flex gap-2">
                    <Input
                      id="new-accessory"
                ...
                {accessories && accessories.length > 0 ? (
                  <div className="space-y-2">
                    {accessories.map((accessory) => (
                      <div key={accessory.id} className="flex items-start gap-3 rounded-lg border p-3">
                        <Checkbox checked={accessory.owned} onCheckedChange={() => handleToggleAccessoryOwned(accessory.id)} />
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm">{accessory.name}</p>
                          {accessory.description && (
                            <p className="text-xs text-muted-foreground mt-1">{accessory.description}</p>
                          )}
                          <Badge variant="outline" className="mt-1 text-xs">{accessory.type}</Badge>
                        </div>
                        <Button variant="ghost" size="icon" onClick={() => handleRemoveAccessory(accessory.id)} className="text-muted-foreground hover:text-destructive shrink-0">
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                    ))}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEquipmentToEdit(null)} disabled={isSaving}>Cancel</Button>
              ...
              {equipmentToDelete && (
                <>
                  ...
                  {equipmentToDelete.accessories && equipmentToDelete.accessories.length > 0 && (
                    <> This equipment has {equipmentToDelete.accessories.length} accessor{equipmentToDelete.accessories.length !== 1 ? 'ies' : 'y'} that will also be deleted.</>
                  )}{' '}
