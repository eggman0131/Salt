...
import { CheckCircle, Link as LinkIcon, Loader2, MessageSquare, Send } from 'lucide-react';
...
export const AIModule: React.FC<AIModuleProps> = ({ onRecipeGenerated, initialUserMessage }) => {
  ...
  useEffect(() => {
    if (initialUserMessage && !hasSentInitial.current) {
      ...
      void handleChat(initialUserMessage);
    }
  }, [initialUserMessage]);
  ...
  const handleChat = async (text?: string) => {
    ...
    try {
      const response = await chatForDraft(newMessages);
  ...
  const handleFinalise = async () => {
    ...
    try {
      ...
      const consensusResponse = await summarizeAgreedRecipe(messages);
      ...
      const recipeData = await generateRecipeFromPrompt(
