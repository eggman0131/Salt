 ...
 * Supports inline editing for prep instructions.
 */
...
export const PrepPhaseView: React.FC<PrepPhaseViewProps> = ({ prepGroups, guideId, onPrepGroupsUpdate }) => {
  ...
  const handleUpdateIngredient = (groupId: string, index: number, value: string) => {
    setEditedGroups(editedGroups.map(g =>
      g.id === groupId
        ? { 
            ...g, 
            ingredients: g.ingredients.map((ing, i) => i === index ? value : ing)
          }
        : g
    ));
  };
